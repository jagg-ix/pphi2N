...
\begin{remark}[Why BL is not needed for the leading mass gap]
The BL concentration bound $\Var(u(x)) \le 1/(\kappa N)$ is
\textbf{not needed} for the leading mass gap. The FK bound is
uniform in~$u$, so the average is controlled by the triangle
inequality alone (step~3). BL is needed for the $1/N$
\textbf{corrections}: the actual mass is $m_0 + O(1/N)$, and
BL controls the correction terms. But the existence of a
positive mass gap $m \ge m_0 > 0$ follows from FK + positivity
alone.
\end{remark}
...
\subsection{What is proved (no axioms, no sorries)}
