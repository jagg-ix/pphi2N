...
The quantitative IFT gives a solution $\Phi$ with
$\|\nabla\Phi\| \le C\|F(0)\| \cdot \|DF(0)^{-1}\|$ in a ball
around $u = 0$. For the mass gap, we need the bound to hold
on the BL-relevant region $\|u\|_\infty \lesssim 1/\sqrt{\kappa N}$.

The IFT ball radius is determined by $\|D^2F\| \cdot \|DF(0)^{-1}\|^2$:
the solution exists for $\|u\|$ such that $\|F(0)(u)\|$ is within the
IFT convergence radius. Since $\|F(0)(u)\| = O(\|u\|^3/\sqrt{N})$
and the convergence radius is $O(N^2)$ (from $\|DF(0)^{-1}\| \le c/N$
and $\|D^2F\| = O(1)$), the IFT ball contains the BL-relevant region
for $N$ sufficiently large:
\[
  O(\|u\|^3/\sqrt{N}) \lesssim O(1/(\kappa N)^{3/2}\sqrt{N})
  = O(1/(\kappa^{3/2}N^2)) \ll O(N^2).
\]
\end{remark}
