...
\item \textbf{Hessian of $V_{\rm eff}$}:
  ...
  $-\log|\det(I+i\nabla^2\Phi)|$. At $\Phi = 0$: this is zero.
  Its Hessian at $\Phi = 0$ involves
  $\nabla^2\Tr\log(I+i\nabla^2\Phi)|_{\Phi=0}$, which gives
  second derivatives of $\Phi$ --- i.e., it depends on the
  thimble correction~$S$ from~\eqref{eq:S_fourier}. Since
  $S = O(1/\sqrt{N})$, the correction is $O(1/N)$ relative
  to the $O(N)$ bare Hessian. For $N$ sufficiently large,
  $H \ge \kappa N/2 > 0$.
\end{enumerate}
