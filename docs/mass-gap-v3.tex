...
\item \textbf{Hessian of $V_{\rm eff}$}:
  $\nabla^2 V_{\rm eff} \ge \kappa N$. This differs from the
  classical case by the $\log|\det J|$ contribution, which
  modifies the Hessian by $O(1)$ (from $\nabla^2\Tr\log
  \sim \nabla^2\Tr(\nabla^2\Phi) \sim S'''$, the
  triangle diagram derivative). For $N$ large, the
  $\kappa N$ term dominates.
\end{enumerate}
...
$F(0)$ is explicitly computable and small: $\im f(u+iv_*)$ is
$O(u^3/\sqrt{N})$ per site (the cubic term from the $\Tr\log$),
and $\Tr\arctan(0) = 0$. The Fr\'echet derivative $DF(0)$ maps
$\delta\Phi \mapsto \partial(\im f)/\partial v \cdot \nabla(\delta\Phi)
+ \Tr(\nabla^2(\delta\Phi))$. The first term involves the Hessian
$H$ (positive definite), so $DF(0)$ is a bounded invertible operator
on a space of smooth functionals. By the quantitative IFT:
$\|\Phi\| \le C\,\|F(0)\|/\|DF(0)^{-1}\| = O(u^3/\sqrt{N})$.
The spectral gap and Hessian bounds follow from proximity
to the classical solution (which satisfies them).
\end{proof}
...
Mathlib has implicit function theorems, and the hypotheses
(smoothness from \texttt{contDiffAt\_log\_det}, invertibility
from $H > 0$) are available.
\end{remark}
...
\item \textbf{Green's function decay}: $G_{m_0}(x,0) =
  (-\Delta+m_0^2)^{-1}(x,0) \le (1/m_0^2)\,e^{-m_0|x|}$
  (Fourier analysis on the torus).

\item \textbf{Mass gap}: Chaining steps 1--4:
  $|\langle\ph(x)\ph(0)\rangle| \le (1/m_0^2)\,e^{-m_0|x|}$
...
The constants $m_0$ and $1/m_0^2$ depend on the gap equation
...
\textbf{41 files, 21 axioms, 1 sorry.}
...
\begin{enumerate}
\item \texttt{contour\_deformation}: the connected correlator of the
  \emph{specific} O($N$) LSM measure \texttt{onInteractingMeasure}
  is bounded by $M^{-1}(x,y)$ where $M = -\Delta + m_0^2$.
\item \texttt{green\_exponential\_decay}: $M^{-1}(x,y) \le
  (1/m_0^2)\,e^{-m_0\,\mathrm{dist}(x,y)}$.
\end{enumerate}

\subsection{What is proved (no axioms; 1 sorry in diagonal case)}

\begin{itemize}
\item \textbf{HS identity}: from Mathlib's
  \texttt{fourierIntegral\_gaussian}.
\item \textbf{HS equivalence} (one-site):
  $\int d\sigma\,e^{-S_{\rm HS}} = c\cdot e^{-S_{\rm original}}$
  --- \textbf{fully proved} (was sorry, now via \texttt{push\_cast; ring}).
\item \textbf{Gap equation algebra}: $-2v_* z = m_0^2$, $v_* < 0$,
  singularity distance $|v_*| = m_0^2\sqrt{N}/2$.
...
\item \textbf{Phase cancellation}: $r_1 e^{i\theta} \cdot r_2 e^{-i\theta}$
  is real and positive (polar form, \texttt{cos\textsuperscript{2}+sin\textsuperscript{2}=1}).
\item \textbf{1D diamagnetic inequality}: $a \le \|a + bi\|$
  (from $\|z\|^2 = a^2+b^2 \ge a^2$).
\item \textbf{Lattice Green's function}: $\|G(n)\| \le 1/m^2$
  (12 theorems from Mathlib Fourier API on \texttt{ZMod}).
\item \textbf{Continuum limit}: OS0--OS2 for the O($N$) LSM on $T^2_L$.
\end{itemize}

\subsection{What is axiomatized (21 axioms)}
...
\hline
\texttt{contour\_deformation} & HS + Cauchy + QHJ + FK $\to$ $|C| \le M^{-1}$
  & research \\
\texttt{green\_exponential\_decay} & $M^{-1} \le Ce^{-m_0|x|}$ (Fourier)
  & medium \\
\texttt{resolvent\_complex\_bound} & $\|(M{+}iV)^{-1}\| \le M^{-1}$ (FK)
  & medium \\
\texttt{quantum\_thimble\_exists} & $\Phi$ exists with BL variance
  & hard \\
\hline
6 diamagnetic sub-axioms & heat kernel, Trotter, Laplace transform
  & medium \\
4 continuum limit & porting from pphi2/gaussian-field
  & routine \\
3 matrix calculus & det $C^\infty$, Jacobi, Hessian
  & routine \\
2 contour shift & Cauchy for rectangles (Mathlib)
  & easy \\
1 Green's decay (sharp) & exponential rate on torus
  & medium \\
\hline
\end{tabular}
\end{center}
...
\begin{enumerate}
\item \textbf{Decomposing \texttt{contour\_deformation}}: This single
  axiom packages the entire bridge from the O($N$) measure to the
  thimble analysis. Breaking it into sub-axioms (HS formula,
  Cauchy shift, QHJ existence, FK domination, triangle inequality)
  would make each piece independently verifiable.
...
(Sections~\ref{sec:thimble}--\ref{sec:HJ_solution}) but does not
yet formalize the detailed mathematics:
\begin{itemize}
\item The thimble geometry (Sections~\ref{sec:constant_shift}--\ref{sec:lagrangian}), Hamilton-Jacobi theory
  (Section~\ref{sec:HJ}), and solution theory
  (Section~\ref{sec:HJ_solution}) are documented but axiomatized
  in Lean. The \texttt{quantum\_thimble\_exists} axiom packages
  the IFT existence + BL variance bound.
\item The ``trivial averaging'' argument (step~3 of the proof plan)
  is not separately formalized; Lean packages the entire bridge
  (HS + Cauchy + QHJ + FK + averaging) into the single axiom
  \texttt{contour\_deformation}.
\item The diamagnetic inequality has a proof structure
  (\texttt{DiagmagneticInequality.lean}, 413 lines) but the
  sub-steps (Trotter, heat kernel, Laplace transform) remain
  axioms.
\item The Green's function Fourier analysis
  (\texttt{GreenDecay.lean}, 291 lines) has 12 proved theorems
  but the sharp exponential rate is axiomatized.
\end{itemize}
The Lean development is a \textbf{coherent, compiling scaffold}
for the proof strategy described in this document, not yet a
complete formal derivation of the analytic steps.
