...
\subsection{Proof plan (using the constant shift)}

The constant-shift form of the thimble
(Section~\ref{sec:constant_shift}) simplifies the proof plan
considerably:

\begin{enumerate}
\item \textbf{Contour shift}: Shift from $v = 0$ to $v = v_*$,
  where $v_*$ is determined by the gap equation so that
  $-2v_* z = m_0^2$. This is a flat contour --- Cauchy's theorem
  for the shift is elementary (the integrand is entire in each
  $\sigma(x)$ except for the $\log\det$ singularities, which are at
  $\sigma = ik^2/(2z)$, far from the shifted contour for
  $|v_*| < k_{\min}^2/(2z)$, which is satisfied by construction).

\item \textbf{Real mass in the operator}: On the shifted contour,
  the $\ph$-operator is $-\Delta + m_0^2 + 2i\sigma z$ with
  $\sigma \in \R$ (the shifted variable). The FK bound gives
  $|G_\sigma(x,0)| \le G_{m_0}(x,0) \sim e^{-m_0|x|}$ for
  \textbf{every} $\sigma$-value.

\item \textbf{Residual sign problem}: On the constant-shift contour,
  $\im\,f \ne 0$ (unlike the exact thimble). The residual imaginary
  part is $O(\sigma^3/\sqrt{N})$ per site. The shifted partition
  function is:
  \begin{equation}
    Z = c\int_{\R^{|\La|}} e^{\re f(u+iv_*)}\cdot
      e^{i\,\im f(u+iv_*)}\,d\sigma.
  \end{equation}
  The real part $e^{\re f}$ is a well-behaved (approximately
  log-concave) measure. The phase $e^{i\,\im f}$ is the residual
  sign problem.

\item \textbf{BL for $\sigma$-concentration}: The real part
  $\re\,f(u+iv_*)$ has positive definite Hessian
  $\sim N\cdot H$ near $\sigma = 0$. Apply BL to the measure
  $e^{\re f}\,d\sigma$ (real, positive, log-concave near saddle)
  for concentration: $\Var(\sigma(x)) \le 1/(\kappa N)$.

\item \textbf{Phase control}: The residual phase $e^{i\,\im f}$
  is controlled by BL concentration. Since $\im\,f$ is $O(\sigma^3)$
  and the measure concentrates at width $O(1/\sqrt{N})$:
  $|\im\,f| \lesssim |\La|/N^2$ (total over all sites).
  For $N \gg \sqrt{|\La|}$: the phase is close to~1.

\item \textbf{FK bound + concentration $\to$ mass gap}: Combine
  the per-$\sigma$ FK bound (step~2) with $\sigma$-concentration
  (step~4) and phase control (step~5):
  \begin{equation}
    \E_\sigma\bigl[G_\sigma(x,0)\bigr]
      \approx G_{m_0}(x,0)\cdot(1+O(1/N))
      \sim e^{-m_0|x|}.
  \end{equation}
  Mass gap $m \ge m_0(1 - O(1/N)) > 0$ for $N > N_0$.
\end{enumerate}

\begin{remark}[Comparison with exact thimble]
The constant-shift proof plan avoids the need to characterize the
exact thimble $\mathcal{J}_0$ globally. The price is the residual
sign problem (step~5), which must be controlled perturbatively.
Using the exact thimble would eliminate the residual sign problem
entirely ($\im\,f = 0$), but requires the global analysis of
Section~\ref{sec:thimble}. For large~$N$, the constant-shift
approach should suffice: the residual phase is $O(|\La|/N^2)$,
negligible for $N \gg \sqrt{|\La|}$.
\end{remark}

\begin{remark}
The leading-order thimble is a \textbf{constant imaginary shift}
$v = 0 \to v = v_*$ (Section~\ref{sec:constant_shift}).
This is a flat manifold --- the simplest possible contour deformation.
The exact thimble has $O(1/N)$ curvature corrections from higher-order
terms in~$f$.
